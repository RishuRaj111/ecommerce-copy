...
const placeOrder = async (req,res) => {
    try {
        ...
        console.log(error)
        res.jsor({success:false,message:error.message})
    }
}

// placing order using Strip methods
...
// placing order using razor-Pay methods
...
const verifyRazorpay  = async (req,res) => {
    try {
        const { response: { razorpay_order_id, razorpay_payment_id, razorpay_signature } = {}, userId } = req.body;
        // console.log("Request Body:", JSON.stringify(req.body, null, 2));
        // console.log("Razorpay id"+razorpay_order_id)
        const orderInfo = await razorpayInstance.orders.fetch(razorpay_order_id)
        console.log(razorpay_order_id, orderInfo)
        console
